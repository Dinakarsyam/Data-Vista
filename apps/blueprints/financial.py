...
from apps.models import SessionLocal, Order, Product, Store
...
def get_metrics():
    ...
    try:
        ...
        profit_over_time_query = db.query(
        ...
        revenue_over_time_query = db.query(
            func.date_trunc('month', Order.orderdate).label('month'),
        ...
        sales_by_region_query = db.query(
        ...
        profit_distribution_query = db.query(
            Product.category,
            func.sum(Order.profit).label('total_profit')
        ).join(Product, Order.productid == Product.productid)
        ...
        revenue_vs_profit_query = db.query(
            Product.category,
            func.sum(Order.totalprice).label('total_revenue'),
            func.sum(Order.profit).label('total_profit')
        ).join(Product, Order.productid == Product.productid)
